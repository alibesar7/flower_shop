...
void main() {
  ...
  group('Add product to cart use case', () {
    test(
      'return ApiSuccess when repo return add product to cart success',
      () async {
        ...
        when(
          mockRepo.addProductToCart(product: 'prodId1', quantity: 2),
        ).thenAnswer(
          (_) async => SuccessApiResult<UserCartsModel>(data: fakeData),
        );

        final result =
            await usecase.call(product: 'prodId1', quantity: 2)
    ...
    test('return ApiError when repo return add product to cart fail', () async {
      when(
        mockRepo.addProductToCart(product: 'prodId1', quantity: 2),
      ).thenAnswer(
        (_) async =>
