...
ApiResult<List<AddressModel>> dummyApiResult = SuccessApiResult<List<AddressModel>>(data: []);
...
void main() {
  ...
  group('GetAddressUsecase', () {
    test(
      ...
      () async {
        ...
        when(mockRepo.getAddress(token)).thenAnswer(
          (_) async => SuccessApiResult(data: fakeAddresses),
        );
        ...
        expect(
          (result as SuccessApiResult<List<AddressModel>>).data.length,
          1,
        );
    ...
    test(
      'returns ErrorApiResult when repo fails',
      () async {
        // arrange
        when(mockRepo.getAddress(token)).thenAnswer(
